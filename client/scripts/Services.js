...
Services.GetLikes = function (userId) {
  ...
  if (useCloudCode) {
    ...
    $.get('likes', {sessionToken: Parse.User.current().getSessionToken()}, function (data) {
      likesPromise.resolve(data);
    });
  }
  return likesPromise;
...
Services.GetLikeJ = function (likeId) {
  ...
  if (useCloudCode) {
    ...
    $.get('likej/'+likeId, {sessionToken: Parse.User.current().getSessionToken()}, function (data) {
      likeJPromise.resolve(data);
    });
  }
  return likeJPromise;
